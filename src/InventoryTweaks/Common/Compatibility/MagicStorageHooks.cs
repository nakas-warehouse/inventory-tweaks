...
{
    private delegate Item GetMainZoneItemCallback(CraftingUIState.RecipesPage self, int slot, ref int context);
    ...
    // Makes the item a clone instead of a reference to prevent the same instance being modified with different parameters at once.
    private static Item CraftingUIState_GetMainZoneItem_Hook(GetMainZoneItemCallback orig, CraftingUIState.RecipesPage self, int slot, ref int context)
    {
